...
//	----------------------------------------------------------------------------
//	----------------------------------------------------------------------------
std::vector<IPdbLine*>& PdbSourceFile::GetLines() {
	//	Cleanup Children Symbols collection
	std::vector<IPdbLine*>::iterator it = m_vectorLines.begin();
	for (; it != m_vectorLines.end(); it++) {
		PdbLine* pPdbSymbol = (PdbLine*)*it;
		delete pPdbSymbol;
	}
	m_vectorLines.clear();
	...
	if (m_pIDiaSourceFile) {
		...
		HRESULT hr = m_pIDiaSourceFile->get_compilands(&pIDiaEnumSymbols);
		if (hr == S_OK) {
			LONG lCount = 0;
			...
			for (DWORD dwPos = 0; dwPos < (DWORD)lCount; dwPos++) {
				...
				if (hr == S_OK) {
					PdbLine* pdbLine = new PdbLine(pIDiaSymbol);

					m_vectorLines.push_back(pdbLine);
				}
			}
