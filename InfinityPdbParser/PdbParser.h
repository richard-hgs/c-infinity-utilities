...
#ifdef INFINITYPDBPARSER_EXPORTS
...
namespace PdbParser {
	...
	struct IPdbSourceFile {
		virtual const wstring& GetSourceFileName() = 0;
		virtual DWORD				GetUniqueId() = 0;
		virtual CheckSumType		GetCheckSumType() const = 0;
		virtual const char* GetCheckSum() const = 0;
		/*!
		@brief .	Get the Source Lines
		@return		Is a collection of interface to the Source line file object.*/
		virtual std::vector< IPdbLine* >& GetLines() = 0;
	};

	/*	@brief	A class to handle a symbol of a Source. */
	struct IPdbLine {
		/*!
		@brief		Retrieve the Module name...
		@return ..	Is the Name of the module.*/
